...
function StreamingApp() {
  ...
  useEffect(() => {
    if (selected?.isTrailer && location.pathname === '/') navigate(`/title/featured-${selected.videoId}`)
...
function Row({ title, movies, select, empty, id }) {
  ...
  const scrollRow = (direction) => cardsRef.current?.scrollBy({ left: direction * Math.max(cardsRef.current.clientWidth * .8, 320), behavior: 'smooth' })
  return <section className="row" id={id}><h2>{title}</h2>{movies.length ? <div className="row-carousel"><button className="row-scroll-control row-scroll-control--previous" type="button" aria-label={`Scroll ${title} left`} onClick={() => scrollRow(-1)}>‹</button><div className="cards" ref={cardsRef}>{movies.map((movie) => <button className="movie-card" key={movie.id} onClick={() => select(movie)}><img src={movie.image} alt={`${movie.title} poster`} /><span>{movie.title}</span></button>)}</div><button className="row-scroll-control row-scroll-control--next" type="button" aria-label={`Scroll ${title} right`} onClick={() => scrollRow(1)}>›</button></div> : empty && title !== 'My List' ? <p className="empty">{empty}</p> : null}</section>
}
